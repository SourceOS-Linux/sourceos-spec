#!/usr/bin/env python3
"""Validate the standalone SourceLocator schema.

`payloadDigest` answers 'is this the same payload'. It cannot answer 'which passage
did this come from', so before a locator existed a schema that carried a source
reference could identify the source but never the position: a reader could not
check a claim without re-deriving the whole document.
...
stronger is the failure this file exists to prevent, so most of the checks below
are negative controls — a constraint with no negative control is indistinguishable
from one that is not enforced.
"""
...
SCHEMA = REPO_ROOT / "schemas" / "SourceLocator.json"
...
def main() -> int:
    ...
    def case(label: str, locator: dict, should_pass: bool) -> None:
        errors = sorted(validator.iter_errors(locator), key=lambda e: list(e.path))
        accepted = not errors
    ...
    case("empty sourceRef", {"provenanceVersion": "document-only", "sourceRef": ""}, False)
