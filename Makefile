...
# declared-not-enforced defect at the spec's own reference layer. Self-excluding:
# scans schemas/ only, proves its own teeth via an inline synthetic negative control.
...
validate-trained-classifiers:
	python3 -m pip install --user jsonschema numpy referencing >/dev/null
	python3 tools/validate_trained_classifiers.py

# On-demand: retrain + re-emit the DataClass classifier artifacts (deterministic).
train-dataclass-classifiers:
	python3 -m pip install --user numpy >/dev/null
	python3 tools/train_dataclass_classifiers.py
...
validate-vocab-currency-loop:
	python3 -m pip install --user jsonschema >/dev/null
	python3 tools/validate_vocab_currency_loop.py

# On-demand LIVE currency scan of THIS repo's approved glossary vs specs/*.md.
# Informational (always exit 0) — writes draft proposals to build/vocab-currency-proposals/.
vocab-currency-dogfood:
	python3 -m pip install --user jsonschema >/dev/null
	python3 tools/dogfood_vocab_currency.py
...
# Wired in here because it lives outside tools/, so `pytest tools/ -q` never collects it —
# a self-test nobody runs proves nothing about the PR that shipped it.
...
validate-reversibility-distance-examples:
	python3 -m pip install --user jsonschema >/dev/null
	python3 tools/validate_reversibility_distance_examples.py

validate-capture-receipt-examples:
	python3 -m pip install --user jsonschema >/dev/null
